...
public final class DynamicSchedulerUtil implements ApplicationContextAware, InitializingBean {
    ...
    public static boolean addJob(XxlJobInfo jobInfo) throws SchedulerException {
        ...
        Trigger trigger;
        // 如果cron表达式为空，就设置仅添加Job
        if (StringUtils.hasText(jobInfo.getJobCron())) {
            ...
            trigger = newTrigger().withIdentity(triggerKey).withSchedule(cronScheduleBuilder).build();
            ...
            trigger = newTrigger()
                    .withIdentity(triggerKey).startAt(new Date(Long.MAX_VALUE)).build();
        }

        // schedule : jobDetail + cronTrigger
        Date date = scheduler.scheduleJob(jobDetail, trigger);

        logger.info(">>>>>>>>>>> addJob success, jobDetail:{}, cronTrigger:{}, date:{}", jobDetail, trigger, date);
