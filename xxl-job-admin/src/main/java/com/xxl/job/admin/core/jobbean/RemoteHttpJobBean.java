...
public class RemoteHttpJobBean extends QuartzJobBean {
    ...
    protected void executeInternal(JobExecutionContext context)
        ...
        // failover trigger
        CallBack callback = failoverTrigger(jobInfo.getExecutorAddress(), params, jobLog);
     ...
     * @return
     */
    public CallBack failoverTrigger(String handler_address, HashMap<String, String> handler_params, XxlJobLog jobLog) {
        if (handler_address.split(",").length > 1) {
            ...
            for (String address : handler_address.split(",")) {
                if (StringUtils.isNotBlank(address)) {
                    ...
                    params.put(HandlerParamEnum.TIMESTAMP.name(), String.valueOf(System.currentTimeMillis()));
                    params.put(HandlerParamEnum.ACTION.name(), ActionEnum.BETA.name());
                    CallBack beatResult = HttpUtil.post(HttpUtil.addressToUrl(address), params);
                    failoverMessage += MessageFormat.format("BETA running, <br>>>>[address] : {0}, <br>>>>[status] : {1}, <br>>>>[msg] : {2} <br><hr>", address, beatResult.getStatus(), beatResult.getMsg());
                    if (beatResult.isSuccess()) {
                        jobLog.setExecutorAddress(address);
                        CallBack triggerCallback = HttpUtil.post(HttpUtil.addressToUrl(address), handler_params);
                        triggerCallback.setSuccess();
            ...
            return CallBack.fail(failoverMessage);
        } else {
            jobLog.setExecutorAddress(handler_address);
            CallBack triggerCallback = HttpUtil.post(HttpUtil.addressToUrl(handler_address), handler_params);
